...
def api_feed():
    try:
        ...
        for user in users:
            tabs = list_tabs(user)
            for tab in tabs:
                media = get_tab_media_paged(user, tab, offset=0, limit=1000)  # Increased limit for more complete feed
                for item in media:
...
def api_upload_to_tab(username, tab):
    try:
        ...
        if data and 'folder_name' in data:
            ...
            folder_path = os.path.join(tab_dir, folder_name)
...
def profile_page(username):
    ...
    <head>
      ...
      <style>
        ...
        .album-card { background: #3a3b3c; border-radius: 8px; padding: 12px; min-width: 120px; max-width: 160px; cursor: pointer; margin: 8px; display: inline-block; }
          ...
          if (albums.length > 0) {
            ...
            albums.forEach(album => {
              ...
              card.onclick = () => {
                ...
                backBtn.style = 'background:#2d88ff;color:#fff;border:none;border-radius:8px;padding:8px 20px;font-size:1.08em;font-weight:500;margin-bottom:16px;margin-right:12px;cursor:pointer;box-shadow:0 2px 8px rgba(45,136,255,0.10);display:inline-flex;align-items:center;gap:8px;transition:background 0.18s,color 0.18s;';
